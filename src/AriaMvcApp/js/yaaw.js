...
var YAAW = (function() {
  ...
  return {
    ...
    event_init: function() {
      ...
      $("#ib-file-download").live("click", function() {
        var indexes = [];

        $("#ib-files .select-box.icon-ok").each(function(i, n) {
           indexes.push(n.getAttribute("data-link"));
         });
         if (indexes.length == 0) {
           ARIA2.main_alert("alert-error", "At least one file should be selected.", 5000);
         } else {
             ...
             for(i = 0; i <= indexes.length - 1; i++) {
                 ...
                 {
                    ARIA2.main_alert("alert-error", "The task is not complete.", 5000);
                 };
             }
         };
